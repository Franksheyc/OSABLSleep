...
yef1=yef1(:,1:4);
    ...
    C_train_x = [C_xr1 ftr.pe ftr.kurto ftr.pex zscore(Cwxr2) Cr2 sumweight_tr1  C_yr1/5 pyr yrf1];%用训练结果  Cr2 crf1 yrf1    sumweight_tr Time_r  Cxxr1 wxr1 xx_b    ftr(:,[6 8 10])
    C_test_x =[C_xe1 fte.pe fte.kurto fte.pex zscore(Cwxe2) Ce2 sumweight_te1 C_ye1/5 pye yef1];%
...
%     C_test_x =[C_xe1 fte.pe fte.kurto fte.pex zscore(Cwxe2)  sumweight_te1 C_ye1/5 pye yef1(:,3:7)];%
    [f,f0]=blstimekindt(time_related,length(unique(train_yy)));
% else
...
function [f,f0]=blstimekindt(time_related,m)
    ...
    f=[ [h*ones(1,m)]  [m m 1]  [m*ones(1,h)]  h 2*m h 1 2 m ];%增加的 c( h*ones(1,5) 3 5*ones(1,h-1) 11 10 2 11)  
