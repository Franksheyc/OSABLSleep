...
%     test_xm2=sleep_embeding(be(:,4));
    train_xm1=label2lab(br(:,end));
    test_xm1=label2lab(be(:,end));
    train_xm2=label2lab(br(:,end-1));
    test_xm2=label2lab(be(:,end-1));
    train_xm=zscore([train_xm1 train_xm2]);%1st 2nd position
    test_xm=zscore([test_xm1 test_xm2]);

sumweight_tr=[ar(:,end).*train_xm1 ar(:,end-1).*train_xm2];
sumweight_te=[ae(:,end).*test_xm1 ae(:,end-1).*test_xm2];
